...
router.post('/profilePicUpload', upload.single('file'), (req, res) => {
  if (!req.file) {
    return res.status(400).send('No file uploaded.');
  }
  res.status(200).json({ message: 'File uploaded successfully', imageUrl:`http://localhost:5010/user/profilePic/${req.file.filename}` });
});
...
router.get("/profilePic/:filename", async (req, res) => {
  try {
    ...
    const files = await db.collection('photos.files').findOne({ filename: req.params.filename });
